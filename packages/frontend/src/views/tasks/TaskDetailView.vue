<template>
  <div class="min-h-screen bg-[#050a0f] flex flex-col items-center">
    <!-- Navbar -->
    <header class="w-full h-16 border-b border-white/5 flex items-center px-6 bg-glass sticky top-0 z-20">
      <button 
        @click="router.back()"
    ...
    <main class="w-full max-w-3xl px-6 py-12 flex-1 flex flex-col">
      ...
      <div v-else-if="task" class="space-y-10 animate-fade-in flex-1 flex flex-col">
        <!-- Header -->
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3 relative">
              ...
              >
                ...
                <option value="in_progress">计算中</option>
                <option value="done">已封存</option>
              </select>
              <div 
            ...
            <div class="flex items-center gap-3 relative">
              ...
              >
                <option value="routine">日常任务</option>
                <option value="critical">重要紧急</option>
                <option value="important">重要不紧急</option>
                <option value="urgent">紧急不重要</option>
                <option value="low">不重要也不紧急</option>
              </select>
              <div 
              ...
              >
                <AlertCircle v-if="task.priority === 'critical' || task.priority === 'important'" class="w-4 h-4" />
                <ArrowUpCircle v-else-if="task.priority === 'urgent'" class="w-4 h-4" />
        ...
        <div class="flex flex-wrap gap-6 text-sm text-white/50">
          ...
          <div class="flex items-center gap-2">
            <Clock class="w-4 h-4" />
            <span>构造于: {{ new Date(task.created_at).toLocaleDateString() }}</span>
          </div>
        </div>
      ...
      <div v-else class="text-white/40 text-center mt-20">
        404: 数据节点丢失或访问权限被抑制。
      </div>
    </main>
  </div>
...
import { ArrowLeft, Calendar, Clock, AlertCircle, ArrowUpCircle, Trash2 } from 'lucide-vue-next'
...
const statusTagClass = computed(() => {
  ...
  switch (task.value.status) {
    case 'done': return 'bg-neon/10 text-neon border-neon'
...
const statusText = computed(() => {
  if (!task.value) return ''
  switch (task.value.status) {
    case 'done': return '已封存'
    case 'in_progress': return '计算中'
    default: return '待处理'
  }
})
...
const priorityTagClass = computed(() => {
  ...
  switch (task.value.priority) {
    case 'critical': return 'text-rose-400 border-rose-500'
    case 'important': return 'text-purple-400 border-purple-500'
    case 'urgent': return 'text-amber-400 border-amber-500'
    case 'low': return 'text-emerald-400 border-emerald-500'
    case 'routine': return 'text-blue-400 border-blue-400'
...
async function saveDueDate() {
  ...
  await save('due_at', task.value.due_at)
}

async function save(field: keyof Task, val: any) {
  if (!task.value) return
  try {
