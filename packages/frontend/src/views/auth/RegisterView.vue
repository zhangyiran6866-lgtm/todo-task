...
<template>
  <div class="w-full max-w-md p-8 rounded-2xl bg-[rgba(5,10,15,0.4)] backdrop-blur-xl border border-[rgba(0,243,255,0.15)] z-10 transition-all">
    <h2 class="text-3xl font-bold text-center text-neon drop-shadow-neon mb-8">
      ...
      <form @submit.prevent="handleRegister" class="space-y-6" novalidate>
        <div>
          <label class="block text-sm font-medium text-[var(--text-secondary)] mb-2">昵称</label>
          <div class="relative">
            <input
              v-model="nickname"
              type="text"
        ...
        <div>
          <label class="block text-sm font-medium text-[var(--text-secondary)] mb-2">邮箱</label>
          <div class="relative">
            <input
              v-model="email"
              type="email"
        ...
        <div v-if="errorMsg" class="text-[#ff4444] text-sm mt-2 p-3 bg-[rgba(255,68,68,0.1)] border border-[#ff4444] rounded flex items-center shadow-[0_0_10px_rgba(255,68,68,0.3)] animate-pulse">
        ...
        <div v-if="successMsg" class="text-[#00ffcc] text-sm mt-2 p-3 bg-[rgba(0,255,204,0.1)] border border-[#00ffcc] rounded flex items-center shadow-[0_0_10px_rgba(0,255,204,0.3)] animate-pulse">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="mt-6 text-center text-sm text-[var(--text-secondary)]">
        已有账号？
        <router-link to="/login" class="text-neon hover:underline">立即登录</router-link>
      </div>
    </div>
</template>
