...
| Phase 6 | 项目优化 | 性能 + 稳定性 + 安全 + 工程质量 | ⬜ 未开始 |
...
- [x] 敏感字段脱敏策略（token/password/邮箱局部掩码）
...
**目标**：在已可用的基础上，系统性提升性能、稳定性、安全性和可维护性

### 前端性能优化
- [ ] 路由级与重资源模块分包优化（3D 看板、图片标注等），降低首屏包体积
- [ ] 静态资源优化（图片压缩、按需加载、缓存策略）
- [ ] 首屏渲染性能评估与优化（LCP、CLS、TTI 指标基线）
- [ ] 列表与复杂组件渲染优化（避免不必要重渲染，优化大列表体验）

### 后端性能与稳定性优化
- [ ] 核心接口压测与瓶颈定位（认证、任务 CRUD、日志查询）
- [ ] MongoDB 索引复核与慢查询治理（结合真实查询路径）
- [ ] 连接池与超时参数调优（数据库、HTTP Server）
- [ ] 错误分类与重试策略完善（可重试/不可重试边界清晰）

### 安全加固
- [ ] 生产配置安全基线检查（JWT 密钥强度、环境变量覆盖、默认值清理）
- [ ] 接口安全策略补强（限流阈值、鉴权边界、输入校验）
- [ ] 敏感信息治理（日志脱敏覆盖率、错误信息最小化暴露）
- [ ] 依赖安全扫描与漏洞修复（Go / Node 依赖）

### 工程质量与 CI/CD 优化
- [ ] 建立统一质量门禁（lint + typecheck + test）并接入 CI
- [ ] 补齐关键路径自动化回归测试，设置最低覆盖率目标
- [ ] Docker 镜像与构建流程优化（体积、构建速度、缓存命中）
- [ ] 发布流程标准化（版本号、变更日志、回滚预案）

### 可观测性与运维优化
- [ ] 增加关键业务指标监控（请求量、错误率、P95/P99 延迟）
- [ ] 日志检索体验优化（常用筛选预设、排障路径）
- [ ] 健康检查与告警策略完善（服务存活、数据库连通、异常峰值）
- [ ] 备份恢复演练制度化（定期演练与恢复时长记录）

### 体验与产品细节优化
- [ ] 关键交互链路打磨（登录、建任务、筛选、编辑、删除）
- [ ] 异常场景体验优化（空状态、弱网、请求失败提示）
- [ ] 移动端细节优化（触控反馈、布局稳定性、可读性）
- [ ] 可访问性基础优化（键盘可达性、语义结构、对比度）

**DoD**：完成优化清单的核心高优项并形成基线报告（性能、安全、稳定性），线上关键指标达到目标阈值且具备持续监控能力
